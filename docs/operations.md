...
`lualock` is centered on sandbox work in Lua. The technical target is: Run capability-filtered plugins with deterministic resource policy checks.
...
The score is deterministic. The current threshold is 153. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 5.
